        ...
        if is_connected():
            try:
                for i, message in enumerate(messages):
                    ...
                    if response.ok:
                        print(f"\033[1;32m[SENT] {full_message}  | Time: {current_time}")
                    else:
                        print(f"\033[1;31m[FAILED] Token Error or Limit | Time: {current_time}")
                    time.sleep(speed)

                print("\033[1;33m[+] Loop completed. Restarting...")
            except Exception as e:
                print(f"\033[1;31m[!] Exception: {e}")
        else:
            print("\033[1;34m[!] No Internet. Sleeping...")
            while not is_connected():
                time.sleep(5)
            print("\033[1;32m[+] Internet Back. Resuming...")

def main():
    token_file = input(BOLD + CYAN + "Token File Path => ").strip()
    convo_id = input(BOLD + CYAN + "Conversation ID => ").strip()
    messages_file = input(BOLD + CYAN + "Messages File Path => ").strip()
    custom_name = input(BOLD + CYAN + "Custom Name Prefix => ").strip()
    speed = int(input(BOLD + CYAN + "Delay Between Messages (sec) => ").strip())
